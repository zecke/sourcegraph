...
export const SiteAdminGenerateProductLicenseForSubscriptionForm: React.FunctionComponent<Props> = ({
    ...
    const [creationOrError, setCreationOrError] = useState<
        null | Pick<GQL.IProductSubscription, 'id'> | typeof LOADING | ErrorLike
    >(null)
    ...
    }, [formData.expiresAt, formData.tags, formData.userCount, onGenerate, submits, subscriptionID])
    ...
    const disableForm = Boolean(creationOrError === LOADING || (creationOrError && !isErrorLike(creationOrError)))

    return (
        <div className="site-admin-generate-product-license-for-subscription-form">
            {creationOrError && !isErrorLike(creationOrError) && creationOrError !== LOADING ? (
            ...
            {isErrorLike(creationOrError) && <ErrorAlert className="mt-3" error={creationOrError} />}
        </div>
    )
}
